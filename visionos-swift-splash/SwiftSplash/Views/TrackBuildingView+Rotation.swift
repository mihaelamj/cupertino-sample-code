...
extension TrackBuildingView {
    ...
    private func createRotationParent(for entity: Entity) {
        ...
        rotationParent?.name = AppState.rotatingParentNodeName
    ...
    func handleRotationChanged(_ value: EntityTargetValue<RotateGesture.Value>, isEnded: Bool = false) {
        guard !(isEnded == true && AppState.isSnapping == true) else { return }
    ...
    private func loopForSnap(info: RotateSnapInfo,
        ...
        while !done {
            ...
            let degreesToAdd = (Date.timeIntervalSinceReferenceDate - info.startTime) * (AppState.rotateSnapTime * 45)
        ...
        if  self.rotationParent?.name != AppState.rotatingParentNodeName {
            self.rotationParent = nil
            AppState.isRotating = false
            return
        }
