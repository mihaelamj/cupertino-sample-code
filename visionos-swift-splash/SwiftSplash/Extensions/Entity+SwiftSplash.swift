...
public extension Entity {
    ...
    func playRideAnimations() {
        ...
        forEachDescendant(withComponent: RideAnimationComponent.self) { entity, component in
            if component.duration > animDuration {
                animDuration = component.duration / AppState.animationSpeedMultiplier
            }
            for animation in entity.availableAnimations {
                ...
                AppState.rideAnimationControllers.append(controller)
                controller.resume()
                controller.speed = Float(AppState.animationSpeedMultiplier)
            }
        }
    ...
    private func loopThroughRidePieces(animDuration: Double) async {
        ...
        while later - adjustedStartTime < animDuration {
            ...
            if AppState.shouldCancelRide { return }
            ...
            if AppState.shouldPauseRide {
                ...
                if rideStartTime > 0 {
                    for controller in AppState.rideAnimationControllers {
                        controller.resume()
                    }
                    AppState.pauseStartTime = 0
    ...
    private func handleRidePause(adjustedStartTime: inout TimeInterval, rideStartTime: inout TimeInterval ) {
        ...
        if AppState.pauseStartTime == 0 {
            AppState.pauseStartTime = Date.timeIntervalSinceReferenceDate
        }
        for controller in AppState.rideAnimationControllers {
            controller.pause()
        }
        ...
        adjustedStartTime = rideStartTime + Date.timeIntervalSinceReferenceDate - AppState.pauseStartTime
    }
