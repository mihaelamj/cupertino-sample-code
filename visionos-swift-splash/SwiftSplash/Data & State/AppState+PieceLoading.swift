...
extension AppState {
    ...
    private func loadTrackPieces(taskGroup: inout TaskGroup<LoadResult>) {
        ...
        for piece in pieces {
            ...
            let pieceSceneName = piece.sceneName
            
            taskGroup.addTask {
                do {
                    guard let pieceEntity = try await self.loadFromRCPro(named: key.rawValue,
                                                                         fromSceneNamed: pieceSceneName,
                                                                         scaleFactor: 1.5) else {
                        fatalError("Attempted to load piece entity \(pieceName) but failed.")
                    }
                    return LoadResult(entity: pieceEntity, key: key.rawValue)
                } catch {
                    fatalError("Attempted to load \(pieceName) but failed: \(error.localizedDescription)")
                }
            }
        }
    }
