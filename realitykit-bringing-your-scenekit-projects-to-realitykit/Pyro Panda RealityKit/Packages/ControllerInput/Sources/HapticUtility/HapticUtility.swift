...
Abstract:
The class that serves as the haptic provider.
*/
...
public final class HapticUtility {
    nonisolated(unsafe) static var engines: [GCController: CHHapticEngine] = [:]

    /// Initializes a new haptic utility object.
    public class func initHapticsFor(controller: GCController?) {
        guard let controller = controller else { return }

        engines[controller] = controller.haptics?.createEngine(withLocality: .default)
    }

    public class func deinitHapticsFor(controller: GCController) {
        engines.removeValue(forKey: controller)
    }

    public class func patternPlayersForHapticsFile(
    ...
    public class func playHapticsFile(named filename: String) {
        guard let controller = GCController.current else { return }
        ...
        do {
            ...
            print("An error occurred playing \(filename): \(error).")
        }
    }
}
