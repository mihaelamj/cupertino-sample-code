...
Abstract:
The class that serves as the game's source of control flow.
*/
...
// MARK: Setup
    ...
    func setupCollisions() {
        // Load the collision mesh from another scene and merge it into the main scene.
    ...
    func setupFollowCamera(_ cameraNode: SCNNode) {
        ...
        // Use a custom constraint to let the player orbit the camera around the character.
    ...
    func setActiveCamera(_ cameraName: String, animationDuration duration: CFTimeInterval) {
        ...
        // Save the old transform in world space.
        let oldTransform: SCNMatrix4 = cameraNode.presentation.worldTransform

        // Reparent the subnode.
        camera.addChildNode(cameraNode)

        // Compute the old transform relative to your new root node.
    ...
    // The game runs triggers when a character enters a box with the collision mask `BitmaskTrigger`.
    ...
    func trigger(_ triggerNode: SCNNode) {
        ...
        if lastTrigger != triggerNode {
            ...
            // The first trigger can't animate (it's the initial camera position).
    ...
    func animateFriends() {
        ...
        SCNTransaction.begin()
        for idx in 0..<friendCount {
            // Unsynchronize the friends' animations.
    ...
    func addFriends(_ count: Int) {
        ...
        // Remove physics from the friends.
    ...
    func unlockDoor() {
        ...
        if friendsAreFree {  // The friends are already unlocked.
            return
        }
    ...
    func collect(_ collectable: SCNNode) {
        if collectable.physicsBody != nil {
            ...
            // The item isn't collectable anymore.
            collectable.physicsBody = nil
    ...
    func updateGyro(_ motion: GCMotion, deltaTime: TimeInterval) {
        ...
        // See https://developer.apple.com/documentation/gamecontroller/gcmotion for information on the game controller's coordinate system.
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        ...
        // Update the internal current controller.
        ...
        // Update the shake gesture recognizer.
    ...
    // Configure for low-power devices only.
