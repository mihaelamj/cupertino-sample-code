...
Abstract:
The class that manages the main character of the game, including its animations, sounds, and direction.
*/
...
class Character: NSObject {
    ...
    private func loadCharacter() {
        // Load the character from an external file.
        ...
        /* Set up the character hierarchy.
         character
         |_orientationNode
         |_model
         */
    ...
    func update(atTime time: TimeInterval, with renderer: SCNSceneRenderer) {
        ...
        // Move the character.
        let directionAllZero = direction.allZero()
        if !directionAllZero {
        ...
        if jumpState == 0 {
            ...
            if touchesTheGround {
                if !wasTouchingTheGroup {
                    ...
                    // Trigger jump particles if the character isn't touching lava.
                    if isBurning {
    ...
    private func handleSlidingAtContact(_ closestContact: SCNPhysicsContact, position start: simd_float3, velocity: simd_float3)
        ...
        // Advance the start position to the nearest point without collision.
