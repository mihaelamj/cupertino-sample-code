...
Abstract:
The class that implements the chasing behavior.
*/
...
class ChaserComponent: BaseComponent {
    ...
    var player: PlayerComponent? {
        didSet {
            ...
            center.append(simd_float2(x: -1, y: 9))
            center.append(simd_float2(x: 1, y: 9))
            center.append(simd_float2(x: 1, y: 11))
            center.append(simd_float2(x: -1, y: 11))

            let path = GKPath(points: center, radius: 0.5, cyclical: true)
    ...
    override func update(deltaTime seconds: TimeInterval) {
        ...
        // If below the chase distance from the enemy, start chasing; otherwise, start wandering.
        switch state {
            case .wander:
