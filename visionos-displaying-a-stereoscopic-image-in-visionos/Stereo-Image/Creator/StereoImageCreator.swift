...
public class StereoImageCreator {
    ...
    @MainActor private func applyTextureToEntity(box: ModelEntity, material: inout ShaderGraphMaterial) async {
        do {
            ...
            let createOptions = TextureResource.CreateOptions(semantic: .color)

            /// The left texture for the shader graph material.
            let leftTexture = try await TextureResource(named: leftFileName, options: createOptions)

            /// The right texture for the shader graph material.
            let rightTexture = try await TextureResource(named: rightFileName, options: createOptions)
