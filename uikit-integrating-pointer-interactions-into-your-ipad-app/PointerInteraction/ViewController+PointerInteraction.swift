...
Abstract:
View controller's UIPointerInteractionDelegate to handle all pointer interactions.
*/
        ...
        if let shapeView = interaction.view as? ShapeView {
            if let targetedPreview = shapeView.targetedPreview() {
                ...
                if shapeView is OvalShapeView {
                    ...
                    let pointerShape = UIPointerShape.path(shapeView.innerPath)
                    ...
                    // Shape the pointer to match the inner path of this view.
                    let roundedRect = shapeView.convert(shapeView.viewPath.bounds, to: view)
                    let pointerShape =
                        UIPointerShape.roundedRect(roundedRect, radius: RoundrectShapeView.cornerRadious)

                    pointerStyle = UIPointerStyle(effect: pointerEffect, shape: pointerShape)
                } else if shapeView is TriangleShapeView {
                    ...
                    */
                    let pointerHoverEffect = UIPointerEffect.hover(targetedPreview, preferredTintMode: .none, prefersShadow: true)
 
                    // Shape the pointer to a special triangle bezier path.
                    let pointerShape = UIPointerShape.path(trianglePointerShape())
    
                    pointerStyle = UIPointerStyle(effect: pointerHoverEffect, shape: pointerShape)
                    
                    // or simply:
                    //return UIPointerStyle(shape: pointerShape, constrainedAxes: [])
                } else {
                    // For non-subclassed ShapeViews, we use automatic effect.
