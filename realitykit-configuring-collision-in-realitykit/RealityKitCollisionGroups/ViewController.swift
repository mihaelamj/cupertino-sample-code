...
class ViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    @IBAction func tappedResetButton(_ sender: Any) {
        ...
        // Because movableEntities is an array of arrays, we use flatmap to
    ...
    /// Loops through all movable entities in the scene and changes their physics mode to specified value
    ...
    fileprivate func setCollisionFilters() {
        // Define four CollisionGroups using different bitmask values
    ...
    fileprivate func configureEntityAccessibility() {
        ...
        // The properties `accessibilityLabelKey` and `accessibilityCustomContent`
        // are not available prior to iOS 17, so we only set them if we're
        // running on that release or later.  In addition to setting the label
        // and description, you must also set `isAccessibilityElement`
        // to `true` in order for the entity to be available in VoiceOver.
        if #available(iOS 17.0, *) {
            let cubeLabel: LocalizedStringResource = "green cube"
            let cubeDescription = [AccessibilityComponent.CustomContent(label: "Cube Description",
                                                                        value: "A green metallic cube you can move.",
                                                                        importance: .default)]
            cube1.isAccessibilityElement = true
            cube1.accessibilityLabelKey = cubeLabel
            cube1.accessibilityCustomContent = cubeDescription
            
            cube2.isAccessibilityElement = true
            cube2.accessibilityLabelKey = cubeLabel
            cube2.accessibilityCustomContent = cubeDescription
            cube3.isAccessibilityElement = true
            cube3.accessibilityLabelKey = cubeLabel
            cube3.accessibilityCustomContent = cubeDescription
            
            let beveledCubeLabel: LocalizedStringResource = "blue beveled cube"
            let beveledCubeDescription = [AccessibilityComponent.CustomContent(label: "Cube Description",
                                                                               value: "A blue cube with beveled corners that can be moved.",
                                                                               importance: .default)]
            beveledCube1.isAccessibilityElement = true
            beveledCube1.accessibilityLabelKey = beveledCubeLabel
            beveledCube1.accessibilityCustomContent = beveledCubeDescription
            beveledCube2.isAccessibilityElement = true
            beveledCube2.accessibilityLabelKey = beveledCubeLabel
            beveledCube2.accessibilityCustomContent = beveledCubeDescription
            beveledCube3.isAccessibilityElement = true
            beveledCube3.accessibilityLabelKey = beveledCubeLabel
            beveledCube3.accessibilityCustomContent = beveledCubeDescription
            
            let sphereLabel: LocalizedStringResource = "red sphere"
            let sphereDescription = [AccessibilityComponent.CustomContent(label: "Cube Description",
                                                                          value: "A red sphere you can move",
                                                                          importance: .default)]
            sphere1.isAccessibilityElement = true
            sphere1.accessibilityLabelKey = sphereLabel
            sphere1.accessibilityCustomContent = sphereDescription
            sphere2.isAccessibilityElement = true
            sphere2.accessibilityLabelKey = sphereLabel
            sphere2.accessibilityCustomContent = sphereDescription
            sphere3.isAccessibilityElement = true
            sphere3.accessibilityLabelKey = sphereLabel
            sphere3.accessibilityCustomContent = sphereDescription
        }
    }
        ...
        if #available(iOS 17.0, *) {
            ...
            movableEntities.flatMap({ $0 }).forEach {
                collisionSubscriptions.append(arView.scene.subscribe(to: CollisionEvents.Began.self, on: $0) { event in
                    ...
                    // RealityKit won't automatically announce that, so we
                    // manually generate a VoiceOver notification.
                    let announce = ("\(entity.accessibilityLabelKey ?? "Entity") collided with \(otherEntity.accessibilityLabelKey ?? "Entity")")
    ...
    /// Needed for our custom pan gesture recognizer to work. RealityKit installs its own gesture recognizer
    /// (`EntityGestureRecognizers`.) behind the scenes. Our gesture recognizer needs to co-exist
    /// and run simultaneously with that one. Returning `true` allows them to both run together.
    ...
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        // RealityKit will automatically generate some VoiceOver events for our
        // accessibility entities. We supplement those with custom VoiceOver
        // notifications to give the user more feedback. Here, we tell them when
        // they've started dragging an element.
        //
        // Because this is only available on iOS 17 and later, we wrap the code
        // in an availability check
        if #available(iOS 17.0, *) {
            if UIAccessibility.isVoiceOverRunning {
                let announce = "Dragging \(entity.accessibilityLabelKey ?? "entity")"
                UIAccessibility.post(notification: .announcement, argument: announce)
            }
        }
