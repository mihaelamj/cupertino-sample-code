...
class Scene {
    ...
    init(device: MTLDevice) {
        ...
        let quadVertices: [AAPLSimpleVertex] = [
            AAPLSimpleVertex(position: vector_float2(x: -1, y: -1)),
            AAPLSimpleVertex(position: vector_float2(x: -1, y: 1)),
            AAPLSimpleVertex(position: vector_float2(x: 1, y: -1)),
        ...
        quadVertexBuffer = BufferView<AAPLSimpleVertex>(device: device,
                                                        array: quadVertices)

        pointLights = BufferView<AAPLPointLight>(device: device,
        ...
        fairy = BufferView<AAPLSimpleVertex>(device: device,
                                             array: fairyVertices)
    ...
    /// Creates lights and assigns each a different position, speed, and color.
    func populateLights() {
                
        for index in 0..<numberOfLights {
            let (position, speed) = createLightPositionAndSpeed(index)
            ...
            pointLight.light_color = createLightColor()
            pointLights.assign(pointLight, at: index)
        }
    }

    func createLightPositionAndSpeed(_ index: Int) -> (SIMD4<Float>, Float) {
        ...
        let position = SIMD4<Float>(distance * sinf(angle),
                                    height,
                                    distance * cosf(angle),
                                    1)
    ...
    func createLightColor() -> vector_float3 {
        let colorId = .random(in: 0...Int.max) % 3
        if colorId == 0 {
            return vector_float3(x: .random(in: 4...6),
                                 y: .random(in: 0...4),
                                 z: .random(in: 0...4))
        } else if colorId == 1 {
            return vector_float3(x: .random(in: 0...4),
                                 y: .random(in: 4...6),
                                 z: .random(in: 0...4))
        } else {
            return vector_float3(x: .random(in: 0...4),
    ...
    func update() {
        ...
        let viewMatrix = Transform.look(eye: SIMD3<Float>(x: 0, y: 18, z: -50),
                                        target: SIMD3<Float>(x: 0, y: 5, z: 0),
                                        up: SIMD3<Float>(x: 0, y: 1, z: 0))

        let rotatedViewMatrix = viewMatrix * cameraRotationMatrix
        frameData.view_matrix = rotatedViewMatrix
        
        let templeScaleMatrix = Transform.scaleMatrix(SIMD3<Float>(0.1, 0.1, 0.1))
        
        let templeTranslationMatrix = Transform.translationMatrix(SIMD3<Float>(0, -10, 0))
        ...
        frameData.temple_normal_matrix = Transform.normalMatrix(from: frameData.temple_modelview_matrix)
