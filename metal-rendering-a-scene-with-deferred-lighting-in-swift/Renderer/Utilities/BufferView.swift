...
struct BufferView<Element> {
    ...
    ///
    /// This method generates a warning and doesn't have any callers.
...
extension BufferView where Element == AAPLSimpleVertex {
    ...
    init(device: MTLDevice, array: [Element], options: MTLResourceOptions = []) {
        ...
        let length = array.count * MemoryLayout<Element>.stride
        let simpleVertexBuffer = device.makeBuffer(bytes: array,
                                                   length: length,
                                                   options: .storageModeShared)

        guard let simpleVertexBuffer else {
            fatalError("The Metal device can't create a buffer for a simple vertex array.")
        }

        buffer = simpleVertexBuffer
        count = array.count
    }
}
