...
function genSubObjects(
  ...
  // If there is no pendingRenewalInfo, the receipt likely contains
  // only non-consumable purchases
  if (pendingRenewalInfo == null){
    return purchases;
  }
...
function processReceiptJSON(body) {
  ...
  // If no latestReceiptInfo, we have no purchases to process.
  ...
  return { products: subs, trialConsumedForGroup };
}
