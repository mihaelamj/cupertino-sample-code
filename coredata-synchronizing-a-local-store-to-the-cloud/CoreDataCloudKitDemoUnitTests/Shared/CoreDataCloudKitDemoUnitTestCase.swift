...
Abstract:
A test base class that provides access to the Core Data stack.
*/
...
class CoreDataCloudKitDemoUnitTestCase: XCTestCase {
    ...
    func verifyLocationStatus() {
        ...
        case .notDetermined:
            ...
            let expectation = expectation(description: "Waiting for location authorization. Failure likely means the dialog was not tapped in time.")
        ...
        case .restricted, .denied:
            XCTFail("Tests require location access to proceed. Reset the location settings for this app, and allow When In Use authorization.")
...
#if targetEnvironment(macCatalyst)
        XCTAssertEqual(.authorizedAlways, locationManager.authorizationStatus, "Some tests require access to location information.")
#else
        XCTAssertEqual(.authorizedWhenInUse, locationManager.authorizationStatus, "Some tests require access to location information.")
#endif //#if targetEnvironment(macCatalyst)
