...
Abstract:
This test case covers the Core Data stack and verifies the app's data model.
*/
...
class TestCoreDataStack: CoreDataCloudKitDemoUnitTestCase {
    ...
    func testColorTransformer() {
        XCTAssertEqual("ColorTransformer", ColorTransformer.transformerName.rawValue)
        guard let transformer = ValueTransformer(forName: ColorTransformer.transformerName) else {
            XCTFail("Unable to instantiate a value transformer for \(ColorTransformer.transformerName)")
            return
        }
        
        let color = UIColor.blue
        let data = transformer.reverseTransformedValue(color)
        guard let transformedColor = transformer.transformedValue(data) as? UIColor else {
            XCTFail("Failed to deserialize color from transformer.")
            return
        }
        
        XCTAssertEqual(color, transformedColor)
    }
    ...
    func verifyTag(_ entity: NSEntityDescription) {
        ...
        verifyAttribute(named: "color",
                        on: entity,
                        type: .transformable,
                        valueTransformerName: ColorTransformer.transformerName,
