...
@implementation AAPLDownsampleFilter
{
...
- (nullable id <MTLTexture>) executeWithCommandBuffer:(_Nonnull id <MTLCommandBuffer>)commandBuffer
                                         inputTexture:(_Nonnull id <MTLTexture>)inTexture
                                                ...
                                                event:(_Nonnull id <AAPLEventWrapper>)event
{
    ...
    NSAssert(outTexture, @"Failed to allocate on heap, did not request enough resources");
...
@implementation AAPLGaussianBlurFilter
{
    id <MTLDevice> _device;
...
- (instancetype) initWithDevice:(nonnull id <MTLDevice>)device
{
    ...
    // Create horizontal blur kernel function
    id <MTLFunction> function = [defaultLibrary newFunctionWithName:@"gaussianblurHorizontal"];
    
    _horizontalKernel = [device newComputePipelineStateWithFunction:function
                                                              error:&error];
    
    NSAssert(_horizontalKernel, @"Failed creating a compute kernel: %@", error);

    // Create vertical blur kernel function
    function = [defaultLibrary newFunctionWithName:@"gaussianblurVertical"];

    _verticalKernel = [device newComputePipelineStateWithFunction:function
                                                            error:&error];

    NSAssert(_verticalKernel, @"Failed creating a compute kernel: %@", error);
                                                ...
                                                event:(_Nonnull id <AAPLEventWrapper>)event
{
    ...
    for(uint32_t mipmapLevel = 1; mipmapLevel < inTexture.mipmapLevelCount; ++mipmapLevel)
    {
        ...
        NSAssert(intermediaryTexture, @"Failed to allocate on heap, did not request enough resources");
