...
static const NSTimeInterval AAPLMaxFramesPerImage = 300;
...
@implementation AAPLRenderer
{
    ...
    AAPLGaussianBlurFilter *_gaussianBlur;
    AAPLDownsampleFilter   *_downsample;
    
    // Event, controlling sequential operations on GPU
    id<AAPLEventWrapper> _event;
    ...
    // Number of frames to display blurred image
    NSUInteger _blurFrames;
}

/// Initialize with the MetalKit view from which the rendere will obtain a Metal device. This
/// MTKView object is also used to the pixel format and other properties of the drawable
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView
{
    ...
    id <MTLFunction> fragmentProgram = [defaultLibrary newFunctionWithName:@"texturedQuadFragment"];
    id <MTLFunction> vertexProgram = [defaultLibrary newFunctionWithName:@"texturedQuadVertex"];
    ...
    _gaussianBlur = [[AAPLGaussianBlurFilter alloc] initWithDevice:_device];
    _downsample = [[AAPLDownsampleFilter alloc] initWithDevice:_device];
...
- (void) moveImagesToHeap
{
    ...
    // Signal that blits have completed before operating on textures in heap
...
- (void) createScratchHeap:(nonnull id <MTLTexture>)inTexture
{
    ...
    MTLTextureDescriptor *descriptor = [AAPLRenderer newDescriptorFromTexture:inTexture
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    if(!_displayTexture || _blurFrames >= AAPLMaxFramesPerImage)
    {
        // Make memory of the display texture usable by by new objects so that the filter operations
    ...
    if(renderPassDescriptor != nil)
    {
        ...
        float lod = ((float)_blurFrames / (float)AAPLMaxFramesPerImage) * _displayTexture.mipmapLevelCount;
