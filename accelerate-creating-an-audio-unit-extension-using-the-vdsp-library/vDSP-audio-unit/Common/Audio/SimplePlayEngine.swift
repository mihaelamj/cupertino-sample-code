...
Abstract:
The vDSP audio unit simple play engine file.
*/
...
public class SimplePlayEngine {
    ...
    // Synchronizes starting and stopping the engine and scheduling file segments.
    ...
    // This block is called every render cycle and receives MIDI events.
    ...
    public func connect(avAudioUnit: AVAudioUnit?, completion: @escaping (() -> Void) = {}) {
        ...
        // Break the audio unit to mixer connection.
        ...
        // Pause the player before rewiring it.
        if isPlaying {
            player.pause()
        }
        ...
        if avAudioUnit.wantsAudioInput {
            ...
            // Connect the player to the effect, and the effect to mixer.
