...
Abstract:
Base class for SwiftUI-capable Audio Unit parameter nodes.
*/
...
///
/// This doesn't provide any of Swift's usual type-safety benefits, and may result in fatal errors if the
...
final class ObservableAUParameter: ObservableAUParameterNode, ObservableObject {
    ...
    func onEditingChanged(_ editing: Bool) {
        if editing {
            ...
            // Set the value here again to prompt its `didSet` implementation, so that it can send the appropriate `.release` event.
            value = value
        }
    }
