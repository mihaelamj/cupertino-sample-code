...
struct FinanceUtilities {
    ...
    static func getSpendingTransactions(for transactions: [Transaction]) -> [Transaction] {
    ...
    static func totalForAssetTransactions(_ transactions: [Transaction]) -> Decimal {
    ...
    static func totalForLiabilityTransactions(_ transactions: [Transaction]) -> Decimal {
        transactions.reduce(0) { partialResult, transaction in
            let amount = transaction.transactionAmount.amount
    ...
    static func fetchLastWeekOfTransactions() async throws -> [Transaction] {
        let startOfWeek = Date.startOfWeek
