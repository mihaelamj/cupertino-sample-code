...
Abstract:
An object that writes movies to the person's photos library.
*/
...
actor MediaLibrary {
    ...
    private var isAuthorized: Bool {
        get async {
            ...
            // If the system hasn't determined the person's authorization status,
    ...
    // A template method for writing a change to the person's photos library.
    ...
    private func loadInitialThumbnail() async {
        // Only load an initial thumbnail if the person has already authorized the app to write to the photos library.
        // Deferring this call prevents the app from prompting for authorization when the app starts.
    ...
    private func createThumbnail(for asset: PHAsset) async {
        // Request the generation of a 256 x 256 thumbnail image.
