...
Abstract:
Kernel for culling lights and creating a list of visible lights.
*/

#import "AAPLShaderCommon.h"
#import "AAPLLightCullingShared.h"

constant uint2 gTileSize               [[function_constant(AAPLFunctionConstIndexTileSize)]];

constant uint2 gDispatchSize           [[function_constant(AAPLFunctionConstIndexDispatchSize)]];
...
static bool isLightVisibleFine(AAPLPointLightCullingData lightData, float4 tileBoundingSphere) { return true; }

// More accurate cone vs sphere test. Only used for spot lights.
static bool isLightVisibleFine(AAPLSpotLightCullingData lightData, float4 tileBoundingSphere)
{
...
static bool intersectsFrustumTile(LightCullingData light, float3 lightPosView, float r, AAPLTileFrustum frustum, bool transparent)
{
...
static void LightClustering(uint tileIdx,
                            ...
                            constant AAPLPointLightCullingData * pointLightCullingBuffer,
                            constant AAPLSpotLightCullingData *  spotLightCullingBuffer,
...
// Fills AAPLTileFrustum structure with all the informations about the tile frustum
// that we need to perform culling.
AAPLTileFrustum computeTileFrustum(constant AAPLFrameConstants & frameData,
                                   constant AAPLCameraParams & cameraParams,
...
kernel void traditionalLightClustering(uint3 coordinates        [[thread_position_in_grid]],
                                       ...
                                       device uint8_t * pointLightClusterIndices                    [[buffer(AAPLBufferIndexPointLightIndices)]],
                                       constant uint8_t * pointLightIndicesTransparent              [[buffer(AAPLBufferIndexTransparentPointLightIndices)]],
                                       device uint8_t * spotLightClusterIndices                     [[buffer(AAPLBufferIndexSpotLightIndices)]],
                                       constant uint8_t * spotLightIndicesTransparent               [[buffer(AAPLBufferIndexTransparentSpotLightIndices)]],
                                       ...
                                       constant AAPLPointLightCullingData * pointLightCullingBuffer [[buffer(AAPLBufferIndexPointLights)]],
                                       constant AAPLSpotLightCullingData * spotLightCullingBuffer   [[buffer(AAPLBufferIndexSpotLights)]],
                                       constant uint2 & lightCount                                  [[buffer(AAPLBufferIndexLightCount)]])
{
    ...
    AAPLTileFrustum frustum = computeTileFrustum(frameData, cameraParams,
...
kernel void traditionalLightCulling(uint2 coordinates        [[thread_position_in_grid]],
                                    ...
                                    device uint8_t * pointLightIndicesTransparent                [[buffer(AAPLBufferIndexTransparentPointLightIndices)]],
                                    device uint8_t * spotLightIndices                            [[buffer(AAPLBufferIndexSpotLightIndices)]],
                                    device uint8_t * spotLightIndicesTransparent                 [[buffer(AAPLBufferIndexTransparentSpotLightIndices)]],
                                    constant AAPLFrameConstants & frameData                      [[buffer(AAPLBufferIndexFrameData)]],
                                    constant AAPLCameraParams & cameraParams                     [[buffer(AAPLBufferIndexCameraParams)]],
                                    constant AAPLPointLightCullingData * pointLightCullingBuffer [[buffer(AAPLBufferIndexPointLights)]],
                                    constant AAPLSpotLightCullingData * spotLightCullingBuffer   [[buffer(AAPLBufferIndexSpotLights)]],
                                    constant uint2 & lightCount                                  [[buffer(AAPLBufferIndexLightCount)]],
                                    constant rasterization_rate_map_data * rrData                [[buffer(AAPLBufferIndexRasterizationRateMap), function_constant(gUseRasterizationRate)]],
                                    constant ushort4 * pointCoarseCulledXY                       [[buffer(AAPLBufferIndexPointLightCoarseCullingData)]],
                                    constant ushort4 * spotCoarseCulledXY                        [[buffer(AAPLBufferIndexSpotLightCoarseCullingData)]],
...
kernel void tileInit(threadgroup float *atomicMinMaxZ                     [[ threadgroup(AAPLTileThreadgroupIndexDepthBounds) ]],
                     threadgroup atomic_uint *lightCounts                 [[ threadgroup(AAPLTileThreadgroupIndexLightCounts) ]])
{
...
kernel void tileDepthBounds(imageblock<DepthData> img_blk,
                            ...
                            constant AAPLCameraParams & cameraParams [[ buffer(AAPLBufferIndexCameraParams) ]],
                            threadgroup atomic_uint *atomicMinMaxZ   [[ threadgroup(AAPLTileThreadgroupIndexDepthBounds) ]]
                            )
{
    float2 bounds;
...
kernel void tileLightCulling(uint threadId                  [[ thread_index_in_threadgroup ]],
                             ...
                             constant AAPLFrameConstants & frameData                [[ buffer(AAPLBufferIndexFrameData) ]],
                             constant AAPLCameraParams & cameraParams               [[ buffer(AAPLBufferIndexCameraParams) ]],
                             constant AAPLPointLightCullingData * pointLightBuffer  [[ buffer(AAPLBufferIndexPointLights) ]],
                             constant AAPLSpotLightCullingData * spotLightBuffer    [[ buffer(AAPLBufferIndexSpotLights) ]],
                             constant uint2 & lightCount                            [[ buffer(AAPLBufferIndexLightCount) ]],
                             constant rasterization_rate_map_data * rrData          [[ buffer(AAPLBufferIndexRasterizationRateMap), function_constant(gUseRasterizationRate) ]],
                             ...
                             threadgroup const atomic_uint *atomicMinMaxZ   [[ threadgroup(AAPLTileThreadgroupIndexDepthBounds) ]],
                             threadgroup atomic_uint *lightCounts           [[ threadgroup(AAPLTileThreadgroupIndexLightCounts) ]]
                             )
{
    uint outputIdx = (groupId.x + groupId.y * outputSize.x) * MAX_LIGHTS_PER_TILE;
...
kernel void tileLightCullingHierarchical(uint threadId                  [[ thread_index_in_threadgroup ]],
                                         ...
                                         threadgroup const atomic_uint *atomicMinMaxZ               [[ threadgroup(AAPLTileThreadgroupIndexDepthBounds) ]],
                                         threadgroup atomic_uint *lightCounts                       [[ threadgroup(AAPLTileThreadgroupIndexLightCounts) ]],
                                         threadgroup uint8_t     *pointLightIndicesTransparent      [[ threadgroup(AAPLTileThreadgroupIndexTransparentPointLights)]],
                                         threadgroup uint8_t     *spotLightIndicesTransparent       [[ threadgroup(AAPLTileThreadgroupIndexTransparentSpotLights)]])
{
    uint outputIdx = (groupId.x + groupId.y * outputSize.x) * MAX_LIGHTS_PER_TILE;
...
kernel void tileLightClustering(uint threadId                 [[ thread_index_in_threadgroup ]],
                                ...
                                constant rasterization_rate_map_data * rrData                   [[ buffer(AAPLBufferIndexRasterizationRateMap), function_constant(gUseRasterizationRate) ]],
                                threadgroup const atomic_uint *atomicMinMaxZ                    [[ threadgroup(AAPLTileThreadgroupIndexDepthBounds) ]],
                                threadgroup const atomic_uint *lightCounts                      [[ threadgroup(AAPLTileThreadgroupIndexLightCounts) ]],
                                threadgroup const uint8_t     *pointLightIndicesTransparent     [[ threadgroup(AAPLTileThreadgroupIndexTransparentPointLights)]],
                                threadgroup const uint8_t     *spotLightIndicesTransparent      [[ threadgroup(AAPLTileThreadgroupIndexTransparentSpotLights)]])
{
...
void LightCoarseCulling(uint threadId,
                        constant AAPLFrameConstants&   frameData,
                        constant AAPLCameraParams&     cameraParams,
                        ...
                        device ushort4*                culledBounds)
{
    for(uint i = threadId ; i < lightCount ; i += 64)
    {
        ...
        if(inFrustumMinZ)
        {
            AAPLBox2D projectedBounds = getBoundingBox(lightPosView.xyz, r, nearPlane, cameraParams.projectionMatrix);
...
kernel void kernelSpotLightCoarseCulling(uint threadId                                         [[thread_index_in_threadgroup]],
                                         constant AAPLFrameConstants & frameData               [[buffer(AAPLBufferIndexFrameData)]],
                                         constant AAPLCameraParams & cameraParams              [[buffer(AAPLBufferIndexCameraParams)]],
                                         constant float & nearPlane                            [[buffer(AAPLBufferIndexNearPlane)]],
                                         constant AAPLSpotLightCullingData * cullingData       [[buffer(AAPLBufferIndexSpotLights)]],
                                         constant uint & lightCount                            [[buffer(AAPLBufferIndexLightCount)]],
                                         constant rasterization_rate_map_data * rrData         [[buffer(AAPLBufferIndexRasterizationRateMap), function_constant(gUseRasterizationRate)]],
                                         device ushort4 *culledBounds                          [[buffer(AAPLBufferIndexSpotLightCoarseCullingData)]])
{
...
kernel void kernelPointLightCoarseCulling(uint threadId                                        [[thread_index_in_threadgroup]],
                                          ...
                                          constant AAPLCameraParams & cameraParams             [[buffer(AAPLBufferIndexCameraParams)]],
                                          constant float & nearPlane                           [[buffer(AAPLBufferIndexNearPlane)]],
                                          constant AAPLPointLightCullingData * cullingData     [[buffer(AAPLBufferIndexPointLights)]],
                                          constant uint & lightCount                           [[buffer(AAPLBufferIndexLightCount)]],
                                          constant rasterization_rate_map_data * rrData        [[buffer(AAPLBufferIndexRasterizationRateMap), function_constant(gUseRasterizationRate)]],
                                          device ushort4 *culledBounds                         [[buffer(AAPLBufferIndexPointLightCoarseCullingData)]])
{
    LightCoarseCulling(threadId,
                       frameData,
                       cameraParams,
