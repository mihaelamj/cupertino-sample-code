...
Abstract:
Header for types shared between Metal and ObjC light culling code.
*/

// Helper struct to store view space bounds.
struct AAPLAxisBounds
{
    float3 upper;
    float3 lower;
};
...
static AAPLAxisBounds getBoundsForAxis(float3 axis, float3 center, float radius, float nearZ)
{
...
struct AAPLBox2D
{
    float2 x;
    float2 y;
    AAPLBox2D(float minX, float minY, float maxX, float maxY)
        : x{minX, maxX}
        , y{minY, maxY} {}

    float2 min() { return float2{x.x, y.x}; }
    float2 max() { return float2{x.y, y.y}; }
};
...
static AAPLBox2D getBoundingBox(float3 center,
                                float radius,
                                float nearZ,
                                float4x4 projMatrix)
{
    AAPLAxisBounds x = getBoundsForAxis(float3{ 1, 0, 0}, center, radius, nearZ);
    AAPLAxisBounds y = getBoundsForAxis(float3{ 0, 1, 0}, center, radius, nearZ);
    ...
    return AAPLBox2D(minX, minY, maxX, maxY);
}
