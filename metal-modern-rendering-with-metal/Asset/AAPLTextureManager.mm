...
Abstract:
Implementation of class tracking source (asset) textures.
*/
...
NSUInteger calculateMinMip(const AAPLTextureData* texture, NSUInteger maxTextureSize)
{
...
MTLRegion calculateMipRegion(const AAPLTextureData* texture, NSUInteger mip)
{
...
@implementation AAPLTextureManager
{
...
- (void)addTextures:(NSArray<AAPLTextureData*>*)textures data:(NSData *)data maxTextureSize:(NSUInteger)maxTextureSize
{
...
- (PendingBlit)loadMipsToTexture:(id<MTLTexture>)texture desc:(const AAPLTextureData*)desc textureData:(NSData *)textureData
...
- (void)addMips:(unsigned int)hash desc:(const AAPLTextureData*)desc
...
- (void)dropMips:(unsigned int)hash desc:(const AAPLTextureData*)desc
                   texture:(id<MTLTexture>)texture originalTexture:(id<MTLTexture>)originalTexture
                    minMip:(NSUInteger)minMip currentMip:(NSUInteger)currentMip
...
- (id<MTLTexture>)createTexture:(const AAPLTextureData*)desc baseMip:(NSUInteger)baseMip
{
...
- (void)updateTextureMappings:(id<MTLTexture>)texture
                         desc:(const AAPLTextureData*)desc
