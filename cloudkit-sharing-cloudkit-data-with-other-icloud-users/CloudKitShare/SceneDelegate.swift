...
extension SceneDelegate {
    ...
    private func animateZoneSpinner(start: Bool) {
        ...
        let mainNC = splitViewController.viewControllers.first as? UINavigationController
        if let mainVC = mainNC?.viewControllers.first as? ZoneViewController {
            _ = mainVC.view
            start ? mainVC.spinner.startAnimating() : mainVC.spinner.stopAnimating()
        }
    }
