                              ...
                              VideoSink *videoSink)
{
    ...
    // If frame encoding fails in a conferencing use case, drop any pending
                                          ...
                                          float expectedFrameRate)
{
    ...
    // one by one so it's easier to detect a failure.
...
/// Process video for low-latency conferencing.
...
OSStatus processVideoConferencing(Options *options)
{
    ...
    // rate control.
    encoderSpecification = (__bridge CFMutableDictionaryRef)
        [NSDictionary dictionaryWithObjectsAndKeys: @YES, kVTVideoEncoderSpecification_EnableLowLatencyRateControl, nil];
    if(!encoderSpecification) {
        fprintf(stderr, "Error: encoderSpecification creation failed\n");
        err = -101;
        goto bail;
    }
