...
The [`ViewController`](https://developer.apple.com/documentation/uikit/uiviewcontroller) class first initializes a ``PDFDocument`` instance and sets its delegate to `self`.
...
The delegate, of type ``PDFDocumentDelegate``, implements a ``PDFDocumentDelegate/classForPage()`` method. This method returns [`AnyClass`](https://developer.apple.com/documentation/swift/anyclass) and declares that all instances of ``PDFPage`` for the document presented through ``PDFView`` should instantiate the subclass `WatermarkPage`.
...
`WatermarkPage` subclasses `PDFPage` so that it can override the ``PDFPage/draw(with:to:)`` method. This method is called by `PDFDocument` to draw the page in a `PDFView`. All custom drawing for a PDF page should be done through this mechanism.
