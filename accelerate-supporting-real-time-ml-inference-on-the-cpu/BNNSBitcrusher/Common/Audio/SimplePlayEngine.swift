...
Abstract:
The BNNS bitcrusher simple play engine.
*/
...
/// Wraps an Audio Unit Extension and provides helper functions.
...
public class SimplePlayEngine {
    ...
    // Indicates whether the engine is playing.
    private(set) var isPlaying = false
    
    // The app calls this block for each render cycle and receives MIDI events.
    private let midiOutBlock: AUMIDIOutputEventBlock = { sampleTime, cable, length, data in return noErr }
    
    // Use this block to send MIDI UMP events to the audio unit.
    ...
    private func setupMIDI() {
        if !MIDIManager.shared.setupPort(midiProtocol: MIDIProtocolID._2_0, receiveBlock: { [weak self] eventList, _ in
            ...
            fatalError("Failed to set up Core MIDI")
        }
    }
    ...
    func initComponent(type: String, subType: String, manufacturer: String, completion: @escaping (Result<Bool, Error>, ViewController?) -> Void) {
        ...
        AVAudioUnit.instantiate(with: component.audioComponentDescription,
            ...
            // Load the view controller and call the completion handler.
    ...
    private func setPlayerFile(_ fileURL: URL) {
        do {
            ...
            fatalError("Couldn't create AVAudioFile instance. error: \(error).")
        }
    }
    ...
    private func setSessionActive(_ active: Bool) {
        ...
        do {
            ...
            fatalError("Couldn't set Audio Session active \(active). error: \(error).")
        }
#endif
    }
    ...
    private func startPlayingInternal() {
        ...
        // Assumptions: This is protected by `stateChangeQueue`. The engine isn't playing.
        ...
        do {
            ...
            isPlaying = false
            fatalError("Couldn't start the engine. error: \(error).")
        }
    ...
    private func scheduleEffectLoop() {
        guard let file = file else {
            fatalError("`file` can't be nil in \(#function).")
        }
    ...
    public func connect(avAudioUnit: AVAudioUnit?, completion: @escaping (() -> Void) = {}) {
        ...
        // Break the audio unit from the mixer connection.
        engine.disconnectNodeInput(engine.mainMixerNode)
        
        resetAudioLoop()
        
        // The engine is done with the audio unit; release all references.
        engine.detach(avAudioUnit)
        
        // The internal function to resume playing and call the completion handler.
        ...
        if avAudioUnit.wantsAudioInput {
            ...
            // Connect the player to the effect and the mixer.
