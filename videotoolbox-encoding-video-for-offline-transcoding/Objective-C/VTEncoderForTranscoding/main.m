...
Abstract:
A Video Toolbox encoder sample app for transcoding.
*/
...
static void usage(const char *appName)
{
    ...
    fprintf(stderr, "\t--save-power             : save power during transcoding\n");
...
static void dumpOptions(Options *options)
{
    ...
    fprintf(stderr, "\t--save-power          : %s\n", (options->savePower) ? "yes" : "no");
...
static OSStatus parseOptions(int argc, const char * argv[], Options *options)
{
    ...
    while(argc > 1) {
        ...
        else if(0 == strcmp(argv[1], "--save-power")) {
            options->savePower = true;
            argc--;
            argv++;
        }
...
int main(int argc, const char * argv[])
{
    ...
    @autoreleasepool {
        err = processVideoTranscoding(&options);
        if(err) {
            goto bail;
        }
    }
