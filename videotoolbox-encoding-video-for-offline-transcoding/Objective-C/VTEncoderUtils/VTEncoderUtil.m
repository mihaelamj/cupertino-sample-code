...
OSStatus parseQPModulationLevel(const char *level, int32_t *outLevel)
{
    ...
    if(0 == strcmp("default", level)) {
        *outLevel = kVTQPModulationLevel_Default;
    }
    else if(0 == strcmp("disabled", level)) {
        *outLevel = kVTQPModulationLevel_Disable;
    }
    else {
        err = -1;
    }
...
OSStatus parsePreset(const char *preset, CFStringRef *outPreset)
{
    ...
    if(0 == strcmp("balanced", preset)) {
        *outPreset = kVTCompressionPreset_Balanced;
    }
    else if(0 == strcmp("highQuality", preset)) {
        *outPreset = kVTCompressionPreset_HighQuality;
    }
    else if(0 == strcmp("highSpeed", preset)) {
        *outPreset = kVTCompressionPreset_HighSpeed;
    }
    else {
        err = -1;
    }
...
{
    if(kVTCompressionPreset_Balanced == preset)
        return "balanced";
    else if(kVTCompressionPreset_HighQuality == preset)
        return "highQuality";
    else if(kVTCompressionPreset_HighSpeed == preset)
        return "highSpeed";
    else
        return "unknown";
}
