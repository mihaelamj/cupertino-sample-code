...
kernel void particleCompute(device Vertex *vertices [[ buffer(kBufferIndexMeshPositions) ]], device Vertex *vbo [[ buffer(kBufferIndexVBO) ]], uint vid [[ thread_position_in_grid ]])
{
...
vertex ColorInOut particleVertex(
    ...
    if (sharedUniforms.progress > 0 && (in.category == 0 || in.category == 2 || in.category == 3)) {
        out.color.a = 1 - sharedUniforms.progress;
    }
    return out;
}
...
struct ModelVertex {
    ...
    float2 uv [[ attribute(kVertexAttributeTexcoord) ]];
};
