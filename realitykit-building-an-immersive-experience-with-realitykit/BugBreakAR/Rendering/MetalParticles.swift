...
class MetalParticles {
    ...
    init(_ library: MTLLibrary, _ view: MTKView, _ count: UInt = 100_000) {
        ...
        vertexMap.bytes.assumingMemoryBound(to: Particle.self).update(from: particles, count: particles.count)
        ...
        indexMap.bytes.assumingMemoryBound(to: UInt32.self).update(from: indices, count: indices.count)
    ...
    func update(_ commandQueue: MTLCommandQueue,
        ...
        computeCommandEncoder?.setBuffer(vertexBuffer, offset: 0, index: Int(kBufferIndexVBO.rawValue))
