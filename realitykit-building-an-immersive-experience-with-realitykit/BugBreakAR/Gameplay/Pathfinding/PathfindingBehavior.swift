...
public class PathfindingBehavior {
    ...
    func performRaycast(origin: SIMD3<Float>, direction: SIMD3<Float>) -> Transform? {
        guard let targetTransform = arView.meshRaycast(origin: origin,
                                                       direction: normalize(direction))
        else { return nil }

        // Update debug visualizations
        pathfindingComponent.raycastVizModel?.transform = targetTransform

        let targetNormal = targetTransform.upVector
