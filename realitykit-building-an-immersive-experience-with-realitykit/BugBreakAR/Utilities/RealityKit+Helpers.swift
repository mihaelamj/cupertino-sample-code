...
extension Entity {
    ...
    func playAudioAsync(_ audioFileResc: AudioFileResource) {
        DispatchQueue.global().async { [weak self] in
            guard let self = self, self.parent != nil else { return }
            self.stopAllAudio()
            self.playAudio(audioFileResc)
        }
    }
...
extension ARView {
    ...
    func rkRaycast(origin: SIMD3<Float>, direction: SIMD3<Float>) -> CollisionCastHit? {
        // Use RealityKit raycast to detect the reconstruction mesh
        let collisionResults = scene.raycast(origin: origin, direction: normalize(direction))
        // Filter out non-mesh entities
        let filteredResults = collisionResults.compactMap { $0.entity as? HasSceneUnderstanding != nil ? $0 : nil }
        guard let hit = filteredResults.first else { return nil }

        return hit
    }

    func meshRaycast(origin: SIMD3<Float>, direction: SIMD3<Float>) -> Transform? {
        guard let hit = rkRaycast(origin: origin, direction: direction),
        hit.entity is HasSceneUnderstanding else { return nil }
