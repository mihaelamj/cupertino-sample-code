...
class CustomRotorsElementViewController: NSViewController,
                ...
                for subView in pageCheck.contentView.subviews where subView is NSButton {
                    if let button = subView as? NSButton {
                        if button.cell?.title == fruit {
                            fruitButtonAccessibilityElement = NSAccessibility.unignoredDescendant(of: button) as? NSAccessibilityElementProtocol
    ...
    fileprivate func updateViews() {
        ...
        for index in 0...CustomRotorsElementViewController.MaxPageIndex {
            let pageView = pageViewforPageIndex(pageIndex: index)
            if index == currentFocusedViewIndex {
    ...
    public func rotor(_ rotor: NSAccessibilityCustomRotor,
        ...
        let children = NSAccessibility.unignoredChildren(from: fruitElements)
        _ = children.filter {
