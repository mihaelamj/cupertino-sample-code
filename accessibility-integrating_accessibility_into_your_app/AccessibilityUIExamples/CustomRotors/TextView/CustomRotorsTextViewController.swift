...
class CustomRotorsTextViewController: NSViewController,
    ...
    func buildVocabulary(textStorage: NSTextStorage) {
        ...
        vocabWords = [ NSRange(vocabWord1Range!, in: textView.string),
                       NSRange(vocabWord2Range!, in: textView.string),
                       NSRange(vocabWord3Range!, in: textView.string),
                       NSRange(vocabWord4Range!, in: textView.string),
                       NSRange(vocabWord5Range!, in: textView.string) ]
        
        // Apply the blue bold font vocabulary words.
        let vocabColor = NSColor.blue
        var attrRange = NSRange(vocabWord1Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.foregroundColor, value: vocabColor, range: attrRange)
        textStorage.applyFontTraits(NSFontTraitMask(rawValue: NSFontTraitMask.RawValue(NSFontBoldTrait)), range: attrRange)
        
        attrRange = NSRange(vocabWord2Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.foregroundColor, value: vocabColor, range: attrRange)
        textStorage.applyFontTraits(NSFontTraitMask(rawValue: NSFontTraitMask.RawValue(NSFontBoldTrait)), range: attrRange)
        
        attrRange = NSRange(vocabWord3Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.foregroundColor, value: vocabColor, range: attrRange)
        textStorage.applyFontTraits(NSFontTraitMask(rawValue: NSFontTraitMask.RawValue(NSFontBoldTrait)), range: attrRange)
        
        attrRange = NSRange(vocabWord4Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.foregroundColor, value: vocabColor, range: attrRange)
        textStorage.applyFontTraits(NSFontTraitMask(rawValue: NSFontTraitMask.RawValue(NSFontBoldTrait)), range: attrRange)
        
        attrRange = NSRange(vocabWord5Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.foregroundColor, value: vocabColor, range: attrRange)
        textStorage.applyFontTraits(NSFontTraitMask(rawValue: NSFontTraitMask.RawValue(NSFontBoldTrait)), range: attrRange)
    }
    ...
    func buildHighlighted(textStorage: NSTextStorage) {
        ...
        highlightedNotes = [ NSRange(highlightedNote1Range!, in: textView.string),
                             NSRange(highlightedNote2Range!, in: textView.string),
                             NSRange(highlightedNote3Range!, in: textView.string),
                             NSRange(highlightedNote4Range!, in: textView.string),
                             NSRange(highlightedNote5Range!, in: textView.string),
                             NSRange(highlightedNote6Range!, in: textView.string),
                             NSRange(highlightedNote7Range!, in: textView.string) ]
        
        // Apply the highlighted words.
        let highlightColor = NSColor.yellow
        var attrRange = NSRange(highlightedNote1Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.backgroundColor, value: highlightColor, range: attrRange)
        
        attrRange = NSRange(highlightedNote2Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.backgroundColor, value: highlightColor, range: attrRange)
        
        attrRange = NSRange(highlightedNote3Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.backgroundColor, value: highlightColor, range: attrRange)
        
        attrRange = NSRange(highlightedNote4Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.backgroundColor, value: highlightColor, range: attrRange)
        
        attrRange = NSRange(highlightedNote5Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.backgroundColor, value: highlightColor, range: attrRange)
        
        attrRange = NSRange(highlightedNote6Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.backgroundColor, value: highlightColor, range: attrRange)
        
        attrRange = NSRange(highlightedNote7Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.backgroundColor, value: highlightColor, range: attrRange)
    }
    ...
    func buildMisspelled(textStorage: NSTextStorage) {
        ...
        misspelledWords = [ NSRange(misspelledWord1Range!, in: textView.string),
                            NSRange(misspelledWord2Range!, in: textView.string),
                            NSRange(misspelledWord3Range!, in: textView.string),
                            NSRange(misspelledWord4Range!, in: textView.string) ]
        
        // Apply the underline misspelled words.
        let underlineValue = NSNumber(value: 1)
        var attrRange = NSRange(misspelledWord1Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.underlineStyle, value: underlineValue, range: attrRange)
        
        attrRange = NSRange(misspelledWord2Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.underlineStyle, value: underlineValue, range: attrRange)
        
        attrRange = NSRange(misspelledWord3Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.underlineStyle, value: underlineValue, range: attrRange)
        
        attrRange = NSRange(misspelledWord4Range!, in: textView.string)
        textStorage.addAttribute(NSAttributedString.Key.underlineStyle, value: underlineValue, range: attrRange)
    }
    ...
    func insertTextViewString() {
        ...
        if let font = textStorage.attribute(NSAttributedString.Key.font, at: range.location, effectiveRange: nil) as? NSFont {
...
extension CustomRotorsTextViewController {
    ...
    public func rotor(_ rotor: NSAccessibilityCustomRotor,
        ...
        if !filterText.isEmpty && currentRange?.location == NSNotFound {
        ...
        if currentTextIndex == NSNotFound {
            ...
            if direction == NSAccessibilityCustomRotor.SearchDirection.previous {
                for index in (0...filteredChildren.count).reversed() {
                    let range = Range(filteredChildren[index], in: contentString!)
                    let subString = contentString![range!]
                
                    let matches = subString.localizedCaseInsensitiveCompare(filterText)
                    let index = filteredChildren[index].location
                    
                    let matchesFilterText = (filterText.isEmpty || matches == .orderedSame)
                    if index < currentTextIndex! && matchesFilterText {
                        targetRangeValue = filteredChildren[index]
                        break
                    }
                }
            } else if direction == NSAccessibilityCustomRotor.SearchDirection.next {
                for index in 0..<filteredChildren.count {
                    let range = Range(filteredChildren[index], in: contentString!)
                    let subString = contentString![range!]
