...
class CustomOutlineView: NSView {
    ...
    @objc var selectedRow: Int = 0 {
        didSet {
            ...
            NSAccessibility.post(element: self, notification: NSAccessibility.Notification.selectedRowsChanged)
        }
    }
    ...
    fileprivate func rowForNode(node: OutlineViewNode) -> Int {
        return visibleNodes().firstIndex(of: node)!
    }
    ...
    fileprivate func setExpandedStatus(expanded: Bool, node: OutlineViewNode) {
        if !node.children.isEmpty {
            ...
            if node.expanded {
                NSAccessibility.post(element: accessibilityElementForNode(node: node), notification: NSAccessibility.Notification.rowExpanded)
            } else {
                NSAccessibility.post(element: accessibilityElementForNode(node: node), notification: NSAccessibility.Notification.rowCollapsed)
            }
            ...
            NSAccessibility.post(element: self, notification: NSAccessibility.Notification.rowCountChanged)
        }
    }
    ...
    override func keyDown(with event: NSEvent) {
        ...
        case Character(NSEvent.SpecialKey.upArrow.rawValue)!:
            selectedRow -= 1
        case Character(NSEvent.SpecialKey.leftArrow.rawValue)!, Character(NSEvent.SpecialKey.rightArrow.rawValue)!:
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for rowidx in 0..<visibleNodes().count {
            ...
            let textAttributes = [ NSAttributedString.Key.font: NSFont.systemFont(ofSize: NSFont.systemFontSize),
