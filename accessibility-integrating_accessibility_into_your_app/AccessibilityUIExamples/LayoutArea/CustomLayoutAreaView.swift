...
class CustomLayoutAreaView: NSView, NSAccessibilityLayoutArea {
    ...
    var selectedLayoutItemIndex: Int = -1 {
        didSet {
            ...
            NSAccessibility.post(element: self, notification: NSAccessibility.Notification.focusedUIElementChanged)
            needsDisplay = true
        }
    }
    ...
    var selectedLayoutItem: LayoutItem? {
        didSet {
            ...
            if let itemIndex = layoutItems.firstIndex(of: selectedLayoutItem!) {
    ...
    fileprivate func handleArrowKeys(with event: NSEvent) {
        ...
        guard event.modifierFlags.contains(.numericPad),
            let charactersIgnoringModifiers = event.charactersIgnoringModifiers, charactersIgnoringModifiers.count == 1,
            let char = charactersIgnoringModifiers.first
            else {
        ...
        switch char {
        case Character(NSEvent.SpecialKey.downArrow.rawValue)!:
            bounds.origin.y -= LayoutInfo.LayoutItemMoveDelta
        case Character(NSEvent.SpecialKey.upArrow.rawValue)!:
            bounds.origin.y += LayoutInfo.LayoutItemMoveDelta
        case Character(NSEvent.SpecialKey.leftArrow.rawValue)!:
            bounds.origin.x -= LayoutInfo.LayoutItemMoveDelta
        case Character(NSEvent.SpecialKey.rightArrow.rawValue)!:
            bounds.origin.x += LayoutInfo.LayoutItemMoveDelta
        default: break
        }
    ...
    override func keyDown(with event: NSEvent) {
        guard let charactersIgnoringModifiers = event.charactersIgnoringModifiers, charactersIgnoringModifiers.count == 1,
              let char = charactersIgnoringModifiers.first
        else {
            super.keyDown(with: event)
            return
        ...
        switch char {
        case Character(NSEvent.SpecialKey.tab.rawValue)!:
            if selectedLayoutItemIndex < (NSInteger)(layoutItems.count - 1) {
                selectedLayoutItemIndex += 1
            }
        case Character(NSEvent.SpecialKey.backTab.rawValue)!: // Shift-tab
    ...
    fileprivate func hitTestForHandleAtPoint(point: NSPoint) -> HandlePosition {
        ...
        if selectedLayoutItemIndex >= 0 {
            let cases = Array(HandlePosition.allCases)
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        repeat {
            ...
            case NSEvent.EventType.leftMouseDown, NSEvent.EventType.leftMouseDragged:
                ...
                currentEvent =
                    (window?.nextEvent(matching: eventMask, until: untilDate, inMode: RunLoop.Mode.eventTracking, dequeue: true))!
            default:
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if iterIdx >= 0 {
            let handles = Array(HandlePosition.allCases)
    ...
    override func becomeFirstResponder() -> Bool {
        ...
        if didBecomeFirstResponder {
            ...
            if event?.type == NSEvent.EventType.keyDown {
                guard let charactersIgnoringModifiers = event?.charactersIgnoringModifiers, charactersIgnoringModifiers.count == 1,
                    let char = charactersIgnoringModifiers.first else { return true }
                switch char {
                case Character(NSEvent.SpecialKey.backTab.rawValue)!: // Shift-tab
                    if selectedLayoutItemIndex > 0 {
                        selectedLayoutItemIndex -= 1
