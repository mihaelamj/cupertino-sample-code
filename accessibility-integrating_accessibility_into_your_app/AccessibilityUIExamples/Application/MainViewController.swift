...
Abstract:
This sample's main view controller listing all the Accessibility examples.
*/
...
class MainViewController: NSViewController, NSTableViewDelegate, NSTableViewDataSource {
    ...
    var selectionIndexesObserver: NSKeyValueObservation?
    ...
    override func viewDidLoad() {
        ...
        selectionIndexesObserver = examplesArrayController.observe(\.selectionIndexes, options: .new, changeHandler: {
            [unowned self] (_, _) in
            if self.examplesArrayController.selectionIndex == NSNotFound {
                self.delegate!.didChangeExampleSelection(mainViewController: self, selection: nil)
            } else {
                if self.delegate != nil {
                    let viewControllers = self.examplesArrayController.arrangedObjects as AnyObject
                    if let example =
                        viewControllers.object(at: self.examplesArrayController.selectionIndex) as? Example {
                        self.delegate!.didChangeExampleSelection(mainViewController: self, selection: example)
                    }
                }
            }
        })
    ...
    public func tableView(_ tableView: NSTableView, isGroupRow row: Int) -> Bool {
        ...
        if let example = (examplesArrayController.arrangedObjects as AnyObject).object(at: row) as? Example {
            ...
            result = example.viewControllerIdentifier.isEmpty
        }
        return result
    }
    ...
    public func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        // A group row has no view controller.
        if example.viewControllerIdentifier.isEmpty {
            guard let cell =
