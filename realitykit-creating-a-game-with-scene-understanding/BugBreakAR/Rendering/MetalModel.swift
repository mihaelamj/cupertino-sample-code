...
class MetalModel {
    ...
    private static let log = OSLog(subsystem: appSubsystem, category: "MetalModel")
    ...
    private func setup(_ library: MTLLibrary, _ view: MTKView) {
        ...
        var uniforms = [InstanceUniforms(modelMatrix: float4x4(), color: SIMD4<Float>.zero, fadeType: 0)]
        uniformsBuffer = view.device!.makeBuffer(
            bytes: &uniforms,
            length: MemoryLayout<InstanceUniforms>.size,
    ...
    static func load(device: MTLDevice, name: String, suffix: String = "obj") -> MDLMesh? {
        guard let assetURL = Bundle.main.url(forResource: name, withExtension: suffix) else {
            log.error("Invalid asset name: %s.%s", "\(name)", "\(suffix)")
            return nil
        }
    ...
    func update(transforms: [float4x4], _ fadeType: Int = 0) {
        ...
        let uniforms = uniformsBuffer.contents().assumingMemoryBound(to: InstanceUniforms.self).advanced(by: 0)
