...
class RadarMap {
    ...
    private var camera = CustomCameraComponent()
    ...
    required init(frame: CGRect, from: CGRect) {
        ...
        blurView.frame.origin.y = Constants.radar2DLocation.origin.y
    ...
    public func updateCreatures(_ transforms: [float4x4]) {
        bugTransforms = transforms
    }
    ...
    public func updateCamera(_ arCamera: ARCamera) {
        ...
        case .transitionToMinimap:
            if updateTransition(arCamera, reverse: false, immediate: !Options.showRadarIntro.value) {
