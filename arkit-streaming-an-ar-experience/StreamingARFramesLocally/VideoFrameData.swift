...
struct VideoFrameData: Codable {
    ...
    init(sampleBuffer: CMSampleBuffer, arFrame: ARFrame) {
    ...
    struct TimeStamp: Codable {
        ...
        var time: CMTime {
            CMTime(value: timeValue, timescale: timeScale, flags: CMTimeFlags(rawValue: timeFlags), epoch: timeEpoch)
        }
    }
    ...
    struct TimingInfo: Codable {
        ...
        var timingInfo: CMSampleTimingInfo {
            CMSampleTimingInfo(duration: duration.time, presentationTimeStamp: pts.time, decodeTimeStamp: dts.time)
        }
    }
