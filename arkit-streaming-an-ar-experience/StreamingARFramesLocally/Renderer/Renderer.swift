...
class Renderer: NSObject {
    ...
    init(device: MTLDevice, renderDestination: RenderDestination) {
        ...
        imagePlaneVertexBuffer = BufferView<Float>(device: device, array: Constants.imagePlaneVertexData)
    ...
    func enqueueFrame(pixelBuffer: CVPixelBuffer, presentationTimeStamp: CMTime, inverseProjectionMatrix: Matrix, inverseViewMatrix: Matrix) {
        ...
        frameQueue.sync {
            ...
            if let videoTextureY = createTexture(fromPixelBuffer: pixelBuffer, pixelFormat: .r8Unorm, planeIndex: 0),
               let videoTextureCbCr = createTexture(fromPixelBuffer: pixelBuffer, pixelFormat: .rg8Unorm, planeIndex: 1) {
                let newFrame = Frame(textureY: videoTextureY,
...
extension Renderer: MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        frameQueue.sync {
            ...
            lastDrawnInverseProjectionMatrix = frame.inverseProjectionMatrix
            lastDrawnInverseViewMatrix = frame.inverseViewMatrix
            
            MetalUtils.encodePass(into: commandBuffer, using: renderPassDescriptor, label: "Main Pass") { renderEncoder in
                ...
                MetalUtils.encodeStage(using: renderEncoder, label: "Video Stream Stage") {
                    ...
                    renderEncoder.setFragmentTexture(CVMetalTextureGetTexture(frame.textureY), index: Int(kTextureIndexY.rawValue))
                    renderEncoder.setFragmentTexture(CVMetalTextureGetTexture(frame.textureCbCr), index: Int(kTextureIndexCbCr.rawValue))
