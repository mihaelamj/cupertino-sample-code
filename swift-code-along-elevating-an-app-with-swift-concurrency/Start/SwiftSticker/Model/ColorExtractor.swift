...
Abstract:
A data model that computes colors from an image using the k-means algorithm, which finds the k dominant colors by k random centroids.
*/
...
nonisolated final class ColorExtractor {
    ...
    nonisolated func extractColors(from data: Data) -> PhotoColorScheme? {
...
private nonisolated extension ColorExtractor {
    ...
    func updateCentroids() -> Bool {
        ...
        return pixelCounts.elementsEqual(centroids.map { return $0.pixelCount }) { lhs, rhs in
            return abs(lhs - rhs) < ColorExtractor.tolerance
        }
    }
