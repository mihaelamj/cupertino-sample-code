...
public class OrderSoupIntentHandler: NSObject, OrderSoupIntentHandling {
    ...
    public func resolveQuantity(for intent: OrderSoupIntent, with completion: @escaping (OrderSoupQuantityResolutionResult) -> Void) {
        ...
        // If the user asks to order more soups than there are in stock,
        // provide a specific response informing the user why the order can't be fulfilled.
        ...
        // Ask the user to confirm that they actually want to order five or more soups.
    ...
    public func confirm(intent: OrderSoupIntent, completion: @escaping (OrderSoupIntentResponse) -> Void) {
        /*
        The confirm phase provides an opportunity to perform any final validation of the intent parameters and to
        ...
        if menuItem.attributes.contains(.available) == false {
            //  An example of how to use a custom response for a failure case when a particular soup item is unavailable.
            completion(OrderSoupIntentResponse.failureOutOfStock(soup: soup))
            return
        }
        
        // After validating the intent order is available, indicate that the intent is ready to handle.
    ...
    public func handle(intent: OrderSoupIntent, completion: @escaping (OrderSoupIntentResponse) -> Void) {
        ...
        //  For the success case, indicate a wait time so the user knows when their soup order will be ready.
