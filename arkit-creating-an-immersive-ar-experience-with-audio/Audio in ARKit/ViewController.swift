...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    private func updatePreviewNode() {
        ...
        // Perform raycasting only when ARKit tracking is in a good state.
        if let camera = sceneView.session.currentFrame?.camera, case .normal = camera.trackingState,
            let result = sceneView.smartRaycast( point, infinitePlane: true, objectPosition: preview.simdWorldPosition, allowedAlignments: [.horizontal] ) {
