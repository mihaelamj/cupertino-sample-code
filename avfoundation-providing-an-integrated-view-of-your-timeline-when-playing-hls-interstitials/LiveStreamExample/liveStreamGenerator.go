...
/*
    Usage:
		$ go run liveStreamGenerator.go --http :[port] --timeScale [scale] --segmentDuration [segmentDur] --windowDuration [windowDur] map.mp4 segment1.mp4 segment2.mp4 ...
		...
		map.mp4 segment1.mp4 segment2.mp4 ... is the map file followd by a list of continuous segments that will form the stream

    Assumptions:
		- Subsequent non-parameter arguments are paths to the map file and continuous segments, in order
...
const (
	...
	mapQParm            = "map"        // Only present in media initialization segment URLs
...
var mapBytes []byte            // Byte array of map file
var segmentBytes [][]byte      // Byte arrays of segments
var numBufferSegments int      // Number of segments to initially buffer

// Command line parameters
var httpAddr string
var timeScale int       // Time scale to use
var segmentDuration int // Segment duration time expressed in timeScale
var windowDuration int  // Live window time expressed in timeScale
...
func main() {
	...
	flag.IntVar(&timeScale, "timeScale", 30000, "Timescale for segmentDuration and windowDuration")
	flag.IntVar(&segmentDuration, "segmentDuration", 60060, "Segment duration in timeScale")
	...
	segments = flag.Args()
	totalNumSegments = len(segments) - 1            // First file in segments list is the map file
	segmentBytes = make([][]byte, totalNumSegments) // Initialize empty byte array with total number of segments
	numBufferSegments = 3                           // Initialize the number of segments to initially buffer
	...
	// Loop through map file and all segments
	for i := 0; i < totalNumSegments+1; i++ {
		var err error
		var content []byte
		...
		if i == 0 {
			...
			segmentBytes[i-1] = append(segmentBytes[i-1], content...)
		}
...
func getIndexFile(lastSequenceNum int) string {
	...
	var segmentDurationSec float64 = float64(segmentDuration) / float64(timeScale)   // Segment time in seconds
	var windowSequenceCount = (windowDuration / timeScale) / int(segmentDurationSec) // Number of segments to show in playlist
	...
	fmt.Println("Writing index file gen ", lastSequenceNum, " starting at ", float64(firstSequenceNum)*segmentDurationSec)

	// Create media playlist
	contents += "#EXT-X-VERSION:9\n"
	contents += fmt.Sprintf("#EXT-X-TARGETDURATION:%d\n", int(segmentDurationSec+1.0))
	contents += fmt.Sprintf("#EXT-X-MEDIA-SEQUENCE:%d\n", firstSequenceNum)
	contents += fmt.Sprintf("#EXT-X-MAP:URI=\"%s?%s=yes\"\n", "map.mp4", mapQParm)
	...
	for sequenceNumber := firstSequenceNum; sequenceNumber <= lastSequenceNum; sequenceNumber++ {
		...
		if segmentOffsetFromSequenceNum(sequenceNumber) == 0 {
			...
			var durationUpToSegment = time.Duration(segmentDurationSec * float64(sequenceNumber) * float64(time.Second))
...
func writeIndexLoop(indexFilePath string) {
	for {
		var now = time.Now()
		var lastSequenceNum = sequenceNumberFromTime(now) + numBufferSegments
...
// Convert integer to a big endian value
func convertIntToBigEndian(num int) []byte {
	// Take a big endian uint64 return it as a big-endian [4]byte
	result := make([]byte, 8) // sizeof uint64
	binary.BigEndian.PutUint64(result[0:], uint64(num))
	return result
}
...
func pokeTimeIntoFragment(fragment []byte, sequenceNum int) []byte {
	...
	for len(remainder) > 0 {
		...
		if found {
			...
			var newTimeStamp = convertIntToBigEndian(sequenceOffset)
			updatedFragment = append(updatedFragment, newTimeStamp...)
			remainder = after[8:] // Cut out the old timestamp (which is occupies [0:8])
		} else {
			break
		}
	}
