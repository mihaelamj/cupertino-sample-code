...
ClockKit can request batches of timeline entries up to that deadline. 
...
the app needs to instantiate a template for the specified identifier and family, 
...
covering all the families that the app supports. 
...
The `drinkDataUpdated()` method starts by saving the app's data. Next, it accesses the list of active 
...
`drinkDataUpdated()` method before the complication server reattaches to the active complications.
...
property. If the property has a non-`nil` value, it cancels the observer and returns the value. 
Otherwise, it waits for the observer, and then returns the value.

## Schedule background observer queries
  
Before Coffee Tracker can communicate with HealthKit, it needs to authorize 
...
Coffee tracker creates the background observer query immediately after it launches.
Setting up an observer query quickly after launch enables the observer to respond promptly
when the system launches it in the background because of a change in HealthKit.
...
This method checks the update for errors. If there aren't any errors, it asynchronously loads
