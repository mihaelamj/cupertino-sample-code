...
Abstract:
Metal shaders for rendering a simple material.
*/
                          ...
                          texture2d<half>          specularMap     [[ texture(AAPLTextureIndexSpecular) ]])
{
    ...
    int num_lights = min(atomic_load_explicit(&tile_data->numLights,memory_order_relaxed),AAPLMaxLightsPerTile);
    ...
    half3 eye_normal     = normalize(tangent_normal.x * in.tangent + tangent_normal.y * in.bitangent + tangent_normal.z * in.normal );
    float4 world_space_normal = frameData.viewMatrixInv * float4(eye_normal.x,eye_normal.y,eye_normal.z,0);
    
    // Calculate directional light contribution.
    float directional_light_intensity = max((float)dot((float3)world_space_normal.xyz,-frameData.directionalLightDirection),(float)0.0f);
    ...
    for(int i = 0; i < num_lights; i++)
    {
        ...
        device AAPLPointLight &light = light_data[lightId];
