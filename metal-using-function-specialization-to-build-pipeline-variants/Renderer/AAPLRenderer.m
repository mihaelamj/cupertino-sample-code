...
@implementation AAPLRenderer
{
    ...
    // Metal objects
    id<MTLBuffer> _frameDataBuffers[AAPLMaxFramesInFlight][AAPLNumViewports];
    ...
    // Current set of buffers to fill with dynamic frame data and set for the current frame
    //   This is the current frame number modulo AAPLMaxFramesInFlight
    uint8_t _frameDataBufferIndex;

    // Frame number to use with _frameDataBufferIndex.
    uint32_t _frameNum;
    ...
    // The _time adjusts how the image is rendered.
    // _time is advanced per frame.
    float _time;
    ...
    // Arrays of shader functions, each index specifying shaders for a given LOD
...
{
    BOOL hasBaseColorMap = [AAPLMesh isTexturedProperty:AAPLFunctionConstantBaseColorMapIndex atQualityLevel:quality];
    BOOL has_normal_map = [AAPLMesh isTexturedProperty:AAPLFunctionConstantNormalMapIndex atQualityLevel:quality];;
    BOOL has_metallic_map = [AAPLMesh isTexturedProperty:AAPLFunctionConstantMetallicMapIndex atQualityLevel:quality];;
    BOOL has_roughness_map = [AAPLMesh isTexturedProperty:AAPLFunctionConstantRoughnessMapIndex atQualityLevel:quality];;
    BOOL has_ambient_occlusion_map = [AAPLMesh isTexturedProperty:AAPLFunctionConstantAmbientOcclusionMapIndex atQualityLevel:quality];;
    BOOL has_irradiance_map = [AAPLMesh isTexturedProperty:AAPLFunctionConstantIrradianceMapIndex atQualityLevel:quality];;
...
{
    for(NSUInteger inFlightIndex = 0; inFlightIndex < AAPLMaxFramesInFlight; inFlightIndex++)
    {
        for(NSUInteger viewportIndex = 0; viewportIndex < AAPLMaxFramesInFlight; viewportIndex++)
        {
            _frameDataBuffers[inFlightIndex][viewportIndex] =
                [_device newBufferWithLength:sizeof(AAPLFrameData)
                                     options:MTLResourceStorageModeShared];

            _frameDataBuffers[inFlightIndex][viewportIndex].label =
                [[NSString alloc] initWithFormat:@"FrameDataBuffer%lu:%lu", inFlightIndex, viewportIndex];
        }
    }
...
{
    _frameDataBufferIndex = _frameNum % AAPLMaxFramesInFlight;

    id<MTLBuffer> leftBuffer = _frameDataBuffers[_frameDataBufferIndex][AAPLViewportLeft];;
    id<MTLBuffer> rightBuffer = _frameDataBuffers[_frameDataBufferIndex][AAPLViewportRight];

    // Update any game state (including updating dynamically changing Metal buffer)
    // Note: writing through these pointers updates the buffer data immediately.
    // That's why we use the _frameDataBufferIndex.
    // Later, while encoding in drawInMTKView, the setVertexBuffer command will
    // queue up a reference to the indicated buffer:
    // i.e.
    //     setVertexBuffer:_frameDataBuffers[_frameDataBufferIndex][viewportIndex] ...
    AAPLFrameData * frameDataLeft = (AAPLFrameData*)leftBuffer.contents;
    AAPLFrameData * frameDataRight = (AAPLFrameData*)rightBuffer.contents;

    vector_float3 directionalLightDirection = vector_normalize ((vector_float3){0.f, -6.f, -6.f});

    frameDataLeft->directionalLightInvDirection = -directionalLightDirection;

    frameDataLeft->lightPosition = (vector_float3) {0.f, 60.f, -60.f};
    ...
    frameDataRight->cameraPos = cameraTranslation;

    *frameDataRight = *frameDataLeft;
    ...
    frameDataLeft->modelMatrix = modelMatrixLeft;
    frameDataRight->modelMatrix = modelMatrixRight;
    ...
    //
    //      frameDataLeft->normalMatrix = matrix_invert(matrix_transpose(matrix3x3_upper_left(modelMatrix)))
    //
    //   We can simply take the upper-left 3x3 elements of the model matrix
    frameDataLeft->normalMatrix = matrix3x3_upper_left(modelMatrixLeft);
    frameDataLeft->modelViewProjectionMatrix = matrix_multiply (viewProjectionMatrix, modelMatrixLeft);
    frameDataLeft->normalMatrix = matrix3x3_upper_left(modelMatrixRight);
    frameDataLeft->modelViewProjectionMatrix = matrix_multiply (viewProjectionMatrix, modelMatrixRight);
    _rotation += .01;

    //set the current quality based on distance from camera
    [self calculateQualityAtDistance:-[self translationForFrame:_time]];

    // Determine if we should use the weight for this transition between levels.  If the current
    //   quality level uses the irradiance texture, but the next one only uses a constant, we need
    //   to use the
    //   use a weight.
    if ([AAPLMesh isTexturedProperty:AAPLFunctionConstantIrradianceMapIndex atQualityLevel:_currentQualityLevel] &&
        ![AAPLMesh isTexturedProperty:AAPLFunctionConstantIrradianceMapIndex atQualityLevel:_currentQualityLevel+1])
    {
        frameDataLeft->irradianceMapWeight = _globalMapWeight;
        frameDataRight->irradianceMapWeight = _globalMapWeight;
    }
    else
    {
        frameDataLeft->irradianceMapWeight = 1.0;
        frameDataRight->irradianceMapWeight = 1.0;
    }
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    // Wait to ensure only AAPLMaxFramesInFlight are getting proccessed by any stage in the Metal
    ...
    if(renderPassDescriptor != nil)
    {
        ...
        for (AAPLMesh *mesh in _meshes)
        {
            ...
            for(AAPLSubmesh *submesh in mesh.submeshes)
            {
                ...
                // Set the material constatnts
                [renderEncoder setFragmentBuffer:submesh.materialData
                                          offset:0
                                         atIndex:AAPLBufferIndexMaterialData];
                ...
                for (NSUInteger viewportIndex = 0; viewportIndex < AAPLNumViewports; viewportIndex++)
                {
                    ...
                    [renderEncoder setVertexBuffer:_frameDataBuffers[_frameDataBufferIndex][viewportIndex]
                                            offset:0
                                           atIndex:AAPLBufferIndexFrameData];

                    [renderEncoder setFragmentBuffer:_frameDataBuffers[_frameDataBufferIndex][viewportIndex]
                                              offset:0
                                             atIndex:AAPLBufferIndexFrameData];
    ...
    // Increment to next frame number
    _frameNum++;
}
