                                       ...
                                       texturecube<float> irradianceMap       [[ function_constant(has_irradiance_map) ]])
{
    ...
    {
        float mapWeight = materialData.mapWeights[AAPLTextureIndexBaseColor];
        parameters.baseColor = baseColorMap.sample(linearSampler, in.texCoord.xy);
        parameters.baseColor *= mapWeight;
        float4 uniformContribution = (1.f - mapWeight) * float4(materialData.baseColor, 1.0f);
        parameters.baseColor += uniformContribution;
    }
    else
    {
        parameters.baseColor = float4(materialData.baseColor, 1.0f);
    }

    if(has_normal_map)
    {
        float mapWeight = materialData.mapWeights[AAPLTextureIndexNormal];
    ...
    parameters.viewDir = normalize(frameData.cameraPos - float3(in.worldPos));

    if(has_roughness_map)
    {
        float mapWeight = materialData.mapWeights[AAPLTextureIndexRoughness];
        parameters.roughness = max(roughnessMap.sample  (linearSampler, in.texCoord.xy).x, 0.001f);
        parameters.roughness *= mapWeight;
        float uniformContribution =  (1.f - mapWeight) * materialData.roughness.x;
        parameters.roughness += uniformContribution;
    }
    else
    {
        parameters.roughness = materialData.roughness.x;
    }

    if(has_metallic_map)
    {
        float mapWeight = materialData.mapWeights[AAPLTextureIndexMetallic];
        parameters.metalness = metallicMap.sample  (linearSampler, in.texCoord.xy).x;
        parameters.metalness *= mapWeight;
        float uniformContribution = (1.f - mapWeight) * materialData.metalness.x;
        parameters.metalness += uniformContribution;
    }
    else
    {
        parameters.metalness =  materialData.metalness.x;
    }

    if(has_ambient_occlusion_map)
    {
        float mapWeight = materialData.mapWeights[AAPLTextureIndexAmbientOcclusion];
        ...
        float uniformContribution = (1.f - mapWeight) * materialData.ambientOcclusion;
        parameters.ambientOcclusion += uniformContribution;
    }
    else
    {
        parameters.ambientOcclusion = materialData.ambientOcclusion;
    }
    ...
    if(has_irradiance_map)
    {
        ...
        float mapWeight = frameData.irradianceMapWeight;
        ...
        float3 uniformContribution =  (1.f - mapWeight) * materialData.irradiatedColor;
        parameters.irradiatedColor += uniformContribution;
    }
    else
    {
        parameters.irradiatedColor = materialData.irradiatedColor;
    }

    parameters.lightDir = frameData.directionalLightInvDirection;
                                  ...
                                  constant AAPLFrameData & frameData [[ buffer(AAPLBufferIndexFrameData) ]])
{
    ...
    out.position = frameData.modelViewProjectionMatrix * float4(in.position, 1.0);
    ...
    float3x3 normalMatrix = float3x3(frameData.normalMatrix);
    out.tangent   = normalMatrix * in.tangent;
    out.bitangent = normalMatrix * in.bitangent;
    out.normal    = normalMatrix * in.normal;
    out.worldPos = (float3) (frameData.modelMatrix * float4(in.position, 1.0)).xyz;
