...
> Related session from WWDC23: Session 10229: [Make features discoverable with TipKit](https://developer.apple.com/videos/play/wwdc2023/10229/)
...
Before tips display, they must load into the app. The sample initializes and loads all tips the app uses by calling ``Tips/configure(_:)``. The best practice is to call this once per app session, for example, in the <doc://com.apple.documentation/documentation/swiftui/app/init()> method of your app.
...
3. Invalidate the tip after a person uses the feature. This is done programmatically by calling ``Tip/invalidate(reason:)`` on the tip content instance along with the reason you invalidated the tip. People can also click the close button in the upper right-hand corner of the tip view to dismiss the tip.
...
2. Place the tip on the feature to highlight using the ``SwiftUI/View/popoverTip(_:arrowEdge:action:)`` view modifier, passing in the tip content.
...
To add a button to a tip, the sample code defines actions using the ``Tip/Action`` type in the ``Tip/actions-96ur0`` property of the tip content.
...
The ``Tips/Parameter`` property wrapper also support types that conforms to the `Codable` and `Sendable` protocol.
In this example, the tip displays if the plants tip has more than two favorites, and one of the favorites is a string with the value "Rose".
...
Options control the frequency with which tips display. Define a ``Tip/Option`` in the ``Tip/options-6ndv3`` property of your tip structure and set the frequency for the tip display there.
