...
[protocol_convergence]:https://developer.apple.com/documentation/nearbyinteraction/nisessiondelegate/session(_:didupdatealgorithmconvergence:for:)
[protocol_didValidate]:https://developer.apple.com/documentation/nearbyinteraction/nisessiondelegate/session(_:didinvalidatewith:)
[protocol_didUpdate]:https://developer.apple.com/documentation/nearbyinteraction/nisessiondelegate/session(_:didupdate:)
...
Use [`session(_:didUpdate:)`][protocol_didUpdate] to track changes to peer connectivity, as shown in the sample below:
``` swift
...
When peers drop out of range or their session validity (user permissions) changes, the framework announces these changes using [`session(_:didInvalidateWith:)`][protocol_didValidate] and the app responds accordingly, as shown here:
``` swift
...
When the app sets [`isCameraAssistanceEnabled`](https://developer.apple.com/documentation/nearbyinteraction/ninearbypeerconfiguration/iscameraassistanceenabled) to `true`, the framework provides coaching suggestions through [`session(_:didUpdateAlgorithmConvergence:for:)`][protocol_convergence], as shown here:
