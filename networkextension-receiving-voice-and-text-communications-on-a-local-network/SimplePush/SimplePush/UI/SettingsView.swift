    ...
    var body: some View {
        NavigationView {
            List {
                ...
                Section(header: Text("Local Push Connectivity"), footer: Text("The NEAppPushProvider will remain active and receive incoming calls and messages while this device is on a configured cellular, Wi-Fi, or Ethernet network.")) {
                    NavigationLink(
                    ...
                    HStack {
                        Toggle("Runs on Ethernet", isOn: $viewModel.matchEthernet)
                    }
                    HStack {
                        Text("Active")
                        Spacer()
