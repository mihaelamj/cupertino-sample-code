...
class SettingsViewModel: ObservableObject {
    ...
    @Published var matchEthernet = false
    ...
    init() {
        ...
        matchEthernet = settings.pushManagerSettings.matchEthernet
        ...
        $matchEthernet
            .receive(on: DispatchQueue.main)
            .sink { [weak self] matchEthernet in
                self?.settings.pushManagerSettings.matchEthernet = matchEthernet
                self?.commit()
            }
            .store(in: &cancellables)
    }
