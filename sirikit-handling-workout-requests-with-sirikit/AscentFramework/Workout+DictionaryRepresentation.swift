...
Abstract:
Extends `Workout` to allow it to be represented as and created from an `NSDictionary`.
 This allows us to save and restore `Workout`s in user defaults and `NSUserActivity` objects.
*/
...
extension Workout {
    ...
    public init?(dictionaryRepresentation: [String: AnyObject]) {
        ...
        if let typeValue = dictionaryRepresentation[DictionaryKeys.goal] as? String, typeValue == DictionaryKeys.openGoal {
            self.goal = .open
        } else if let typeValue = dictionaryRepresentation[DictionaryKeys.goal] as? String,
            let duration = dictionaryRepresentation[DictionaryKeys.duration] as? TimeInterval,
            typeValue == DictionaryKeys.timedGoal {
            self.goal = .timed(duration)
        } else {
            return nil
        }
    }
