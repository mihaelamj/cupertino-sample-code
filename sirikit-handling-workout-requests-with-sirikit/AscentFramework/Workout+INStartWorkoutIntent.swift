...
extension Workout {
    ...
    public init?(startWorkoutIntent intent: INStartWorkoutIntent) {
        ...
        @unknown default:
            preconditionFailure("Unexpected value for workoutLocationType.")
        }
        ...
        if let goalValue = intent.goalValue,
            let duration = TimeInterval(workoutGoalValue: goalValue, workoutGoalUnitType: intent.workoutGoalUnitType) {
            self.goal = .timed(duration)
        } else if let isOpenEnded = intent.isOpenEnded, isOpenEnded == true {
            self.goal = .open
        } else {
            return nil
        }
