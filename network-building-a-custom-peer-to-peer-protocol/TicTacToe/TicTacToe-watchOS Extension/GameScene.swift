...
class GameScene: SKScene {
    ...
    func updateSelectedRowColumn(_ tapLocation: CGPoint, tapBounds: CGRect) {
        let rowValue = Int(tapLocation.x / tapBounds.width * 3)
        let columnValue = Int(tapLocation.y / tapBounds.height * 3)
        
        if boardLabels[columnValue][rowValue].text == "" {
