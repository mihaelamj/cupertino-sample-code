...
The app checks the device's supported sample count with [`supportsTextureSampleCount:`](https://developer.apple.com/documentation/metal/mtldevice/1433355-supportstexturesamplecount) and uses a default sample count of four.
...
If the device supports the feature, then the renderer creates a new PSO that uses the `_averageResolveTileKernelFunction`.
