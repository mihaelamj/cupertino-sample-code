...
The [`vDSP.gather(_:indices:)`](https://developer.apple.com/documentation/accelerate/vdsp/3600621-gather) function returns arrays that contain `indices.count` color values.
...
The quantization step uses the [`BNNS.scatter(input:indices:output:axis:reductionFunction:filterParameters:)`](https://developer.apple.com/documentation/accelerate/bnns/3972142-scatter) that performs an operation that's the inverse of the gather function the app uses for calculating k-means.
